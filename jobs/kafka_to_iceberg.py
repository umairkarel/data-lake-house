...
kafka_to_iceberg.py
-------------------
PyFlink job: reads JSON events from Kafka → writes to an Iceberg table
via the Nessie catalog on MinIO.

This replaces KafkaIcebergDataStreamJob.java from the original project.
...
def main():
    ...
    # -------------------------------------------------------------------------
    # 4. Create Kafka source table (Flink SQL connector)
    #    Reads raw JSON from the topic — schema matches msgGeneratorKafka output
    # -------------------------------------------------------------------------
    t_env.execute_sql(f"""
    ...
    statement_set.add_insert_sql(f"""
        ...
        SELECT
            ...
            `value`,
            amount,
            CAST(TO_TIMESTAMP(event_time, 'yyyy-MM-dd''T''HH:mm:ss.SSSSSS') AS TIMESTAMP(6)) AS event_time,
